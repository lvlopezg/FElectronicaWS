...
namespace FElectronicaWS.Servicios
{
	public class notaCredito : InotaCredito
	{
		...
		public string getData(int nroNotaCredito, int idCliente, int nroAtencion, string urlPdfNotaCredito)
		{
			...
			try
			{
				...
				{
					conn.Open();
					string qryNotaCredito = @"SELECT B.IdCuenta,B.NumDocumento,B.IdCausal,B.fecMovimiento,B.FecRegistro,B.ValMonto,
...
and C.IdNumeroNota=@nroNotaCredito2 and A.IndTipoNota='C'
";
					...
					OriginalRequest peticion = new OriginalRequest
					{
						...
						InvoiceId = nroNotaCredito.ToString(),
...
WHERE a.IdMovimiento=@idMovimiento";
					...
					try
					{
						...
						//////        {
						//////            logFacturas.Info("Nota Credito Actualizada en facNotaTempWEBService");
						//////        }
						//////        else
						//////        {
						//////            logFacturas.Info("!!!   No fue posible Actualizar la Nota Credito en facNotaTempWEBService   !!!");
						//////        }
						//////    }
						//////}
						//////return nroNotaCredito.ToString();
						//}
						...
						if (strsb.Contains(validacion))
						{
									...
									if (cmdActualiza.ExecuteNonQuery() > 0)
									{
										...
										using (WebClient webClient = new WebClient())
										{
											try
											{
												...
												string carpetaDescarga = Properties.Settings.Default.urlDescargaPdfNC + DateTime.Now.Year + @"\" + nombreArchivo + ".pdf";
												...
												//webClient.DownloadFile(rptaEntrega.Resultado.ZIP, @"D:\NotaCreditoQR\" + DateTime.Now.Year + @"\" + rptaEntrega.Resultado.CUFE + ".zip");
							...
							{
								logFacturas.Info("!!!  La Recuperacion de Documentos de la Nota Credito, No fue posible. No se Actualiza la Factura en facNotaTempWEBService   !!!");
