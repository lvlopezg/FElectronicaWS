...
WHERE IdFactura =  @nroFactura";
				...
				OriginalRequest peticion = new OriginalRequest
				{
					...
					//TODO:Agregar todos los campos adicionales necesarios.
					...
					//InformacionAdicionalEn.Add(new AdditionalInformation() { Position = "6", Value = nroAtencion.ToString() }); // Nro de Atencion
...
a.IdFactura,F.IdProducto,p.codproducto,p.NomProducto,F.IdMovimiento,F.ValDescuento,F.CantidadFNC,F.IdTasaVenta,F.ValTasa,F.ValCuotaMod,F.indPos,F.Regcum
FROM facfactura a
...
WHERE a.IndTipoFactura='ACT' AND  a.idfactura= @idFactura";
				...
				try
				{
					...
					////////{
					////////    StreamReader streamRpta = new StreamReader(response.GetResponseStream());
					////////    string respuestaStatus = streamRpta.ReadToEnd();
								...
								if (cmdActualiza.ExecuteNonQuery() > 0)
								{
									...
									using (WebClient webClient = new WebClient())
									{
										try
										{
											...
											logFacturas.Info($"Descarga de Factura...Terminada");
											...
											using (SqlConnection conn3 = new SqlConnection(Properties.Settings.Default.DBConexion))
											{
												...
												{
													logFacturas.Info("No fue Posible Realizar la Descarga de Archivos de la Factura con Identificadotr:" + rptaEntrega.Identificador);
												}
											}
										}
